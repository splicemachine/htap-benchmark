...
public abstract class Worker<T extends BenchmarkModule> implements Runnable {
    ...
    synchronized public void cancelStatement() {
        try {
            if (this.currStatement != null && this.wrkld.getDBType() != DatabaseType.SPLICEMACHINE) // Splice Machine does not yet support cancel()
                ...
                // Database System Specific Exception Handling
                	...
                	if (this.wrkld.getDBType().shouldUseTransactions()) {
                		if (savepoint != null) {
                			...
                			LOG.info("Worker SQl Exception rollback: the connection is " + (conn.isClosed() ? "closed" : "open"));
                			this.conn.rollback();
                		}
                	}
