...
public class StockLevel extends TPCCProcedure {
	...
	private PreparedStatement stockGetCountStock = null;
	 ...
	 public ResultSet run(Connection conn, Random gen,
         ...
         // added trace for connection, prepared statements, and result set open/closed for SpliceMachine tests
         if (trace) LOG.trace(String.format(
        		 "StockLevel.run connection is %s, stockGetCountStock is %s",
        		 conn.isClosed() ? "closed" : "open", stockGetCountStock.isClosed() ? "closed" : "open"
        		 ));

         if (trace) LOG.trace(String.format("stockGetCountStock BEGIN [W_ID=%d, D_ID=%d, O_ID=%d, threshold=%d]", w_id, d_id, o_id, threshold));
